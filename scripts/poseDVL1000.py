...
def publishDVLdata():
    ...
    pubPosZ = rospy.Publisher('manta/pose_z', PoseWithCovarianceStamped, queue_size=2)
    # pubWater = rospy.Publisher('sensors/dvl/water', DVL, queue_size=10)
    rospy.init_node('Pose_DVL1000', anonymous=False)
    ...
    if theData["id"] == 8219:
        ...
        theDVL.header.frame_id = "dvl_link1"
        ...
        thePoseZ.header.stamp = rospy.Time.now()
        thePoseZ.header.frame_id = "pressure_link"

        global initZ

        thePoseZ.pose.pose.position.z =-((BottomPressureData*100000)-101325)/(997*9.81) - initZ
        if (initZ == 0) and (theDVL.velocity.x != -32.768002):
            initZ = thePoseZ.pose.pose.position.z

        if theDVL.velocity.x != -32.768002:
            thePoseZ.pose.covariance = [unknown, unknown, unknown, unknown, unknown, unknown,
                                        unknown, unknown, unknown, unknown, unknown, unknown,
                                        unknown, unknown, 0.01 *(10**(-2)), unknown, unknown, unknown,
                                        unknown, unknown, unknown, unknown, unknown, unknown,
                                        unknown, unknown, unknown, unknown, unknown, unknown,
                                        unknown, unknown, unknown, unknown, unknown, unknown]
